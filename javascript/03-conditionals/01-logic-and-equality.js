function main() {
    //Do not change the varialbes themselves, but jump down to the conditional checks.
    let personOne = "Bryce"
    let personTwo = "Bruce"
  
    //This conditional should check that the two variables (personOne, personTwo) are not equal
    //Only change the check portion of the conditional
    if ( /* Your code goes here in place of this comment */  ) {
      let job = true
      let user = {
        name: 'Dylan',
        age: 18,
        employer: 'HP'
      }
  
      if (  /* check that job is true without an operator */ && /* check name is Samantha and age is 18 or that the employer is HP */ )
      {  
        console.log("Congrats you're making amazing progress with conditionals.")        
      }


      //Extra Challenge can you rewrite the above but using a || (or) instead of the &&??
    }
  }
  
  main()